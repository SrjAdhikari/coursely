//* src/templates/emails/_base.ts

/**
 * Shared brand tokens for all Manakuru transactional emails.
 */
const brand = Object.freeze({
	name: "Manakuru",
	logoUrl: "https://manakuru.srjdev.com/email/logo.png",
	primaryColor: "#a3e635",
 ...
 *
 * @returns Complete HTML document ready for dispatch
 */
const layoutEmail = ({
	preheader = "",
	title = "",
	bodyHtml,
}: EmailLayoutParams): string => `<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8" />
		...
		<tr>
			<td align="center">
				<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width: 560px; background-color: ${brand.cardBg}; border: 1px solid ${brand.borderColor}; border-radius: 8px; overflow: hidden;">
					<!-- Brand header: centered logo above the wordmark -->
					<tr>
						<td align="center" style="padding: 28px 32px 20px; border-bottom: 1px solid ${brand.borderColor};">
							...
							<div style="font-size: 24px; font-weight: 700; color: ${brand.textColor}; letter-spacing: 0.2px;">${brand.name}</div>
						</td>
					</tr>
